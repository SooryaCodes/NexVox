...
const ChatTab: React.FC<ChatTabProps> = ({
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* AI Assistant Activation Banner - Moved here above the input */}
      <div className={`p-3 border-t border-b border-white/10 bg-black/60 transition-all duration-300 ${isAiAssistantActive ? "bg-[#FF00E6]/5" : ""}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <svg width="18" height="18" viewBox="0 0 192 192" fill="none" xmlns="http://www.w3.org/2000/svg" className={isAiAssistantActive ? "text-[#FF00E6]" : "text-white/50"}>
            ...
            <div className="flex flex-col">
              <span className={`text-xs font-medium ${isAiAssistantActive ? "text-[#FF00E6]" : "text-white/70"}`}>
                {isAiAssistantActive ? "AI Assistant Active" : "AI Assistant"}
              </span>
              {isAiAssistantActive && (
                <span className="text-xs text-[#FF00E6]/70">Ask about audio, settings, or room features</span>
              )}
            </div>
          </div>
          ...
          <m.button
            ...
            className={`px-3 py-1 rounded text-xs font-medium ${
              isAiAssistantActive 
                ? "bg-[#FF00E6]/20 border-[#FF00E6]/50 text-[#FF00E6]" 
          ...
          >
            {isAiAssistantActive ? "Deactivate" : "Activate"}
          </m.button>
        </div>
      </div>
        ...
        <form onSubmit={handleSendMessage} className="flex gap-2">
          <div className="relative flex-1">
            <input
              ...
              className="w-full bg-black/40 border border-white/20 rounded-md px-3 py-2 focus:outline-none focus:border-[#00FFFF] focus:ring-1 focus:ring-[#00FFFF] transition-colors placeholder-white/40"
            />
          ...
          <m.button
            type="button"
            className="bg-[#9D00FF]/20 border border-[#9D00FF]/50 text-[#9D00FF] p-2 rounded-md flex items-center justify-center"
          ...
          <m.button
            type="submit"
            className="bg-[#00FFFF]/20 border border-[#00FFFF]/50 text-[#00FFFF] p-2 rounded-md flex items-center justify-center"
