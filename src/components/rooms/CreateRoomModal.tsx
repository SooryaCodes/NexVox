...
const CreateRoomModal: React.FC<CreateRoomModalProps> = ({ isOpen, onClose }) => {
    ...
    if (isOpen && modalRef.current) {
      ...
      gsap.fromTo(
        ...
        { 
          scale: 0.9, 
          opacity: 0,
          y: 0 // Start without vertical offset
        },
        { 
          scale: 1, 
          opacity: 1, 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal container - ensure perfect centering */}
      <div className="relative z-10 w-full max-h-[90vh] max-w-[600px] mx-4 sm:mx-auto flex items-center justify-center">
        {/* Modal */}
        <div 
          ref={modalRef}
          className="w-full bg-black/80 backdrop-blur-xl border border-white/10 rounded-xl shadow-[0_0_25px_rgba(0,255,255,0.2)] overflow-auto max-h-[90vh]"
          role="dialog"
          aria-labelledby="create-room-title"
        >
          {/* Modal header */}
          <div className="sticky top-0 z-10 border-b border-white/10 px-6 py-4 flex justify-between items-center bg-black/80 backdrop-blur-xl">
            <h2 id="create-room-title" className="font-orbitron text-[#00FFFF] text-xl sm:text-2xl">
              Create Room
            </h2>
            <button 
              onClick={onClose}
              className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 text-white/60 hover:text-[#FF00E6] hover:bg-white/10 transition-colors"
          ...
          {/* Modal footer */}
          <div className="sticky bottom-0 z-10 border-t border-white/10 px-6 py-4 flex justify-between bg-black/80 backdrop-blur-xl">
            {step > 1 ? (
              <button
                onClick={prevStep}
